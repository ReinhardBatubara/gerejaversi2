            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach ($layanans as $layanan)
                    <div class="border p-4 rounded shadow">
                        <div class="font-semibold">{{ $layanan['nama'] }}</div>
                        <div>Status: 
                            @if ($layanan['status'])
                        ...
                        <form action="{{ route('layanan.toggleStatus', $layanan['kode']) }}" method="POST" class="mt-3">
                            ...
                            <button type="submit" class="text-sm px-3 py-1 rounded border
                                {{ $layanan['status'] ? 'border-red-600 text-red-600 hover:bg-red-100' : 'border-green-600 text-green-600 hover:bg-green-100' }}">
                                {{ $layanan['status'] ? 'Nonaktifkan' : 'Aktifkan' }}
                            </button>
                        </form>
                    </div>
                @endforeach
